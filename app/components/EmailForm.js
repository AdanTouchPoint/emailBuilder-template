...
const EmailForm = ({setDataQuestions,dataQuestions,setQuestions,questions,setShowThankYou, setShowFindForm, dataUser, setDataUser, showEmailForm, setShowEmailForm, emailData, setEmailData, clientId, backendURLBase, endpoints, backendURLBaseServices, mainData}) => {
   ...
   const handleQuestions  = e => {
    ...
    const send = async e => {
        ...
        setError(false)
        const payload = await fetchData('GET', backendURLBaseServices, endpoints.toSendEmails, clientId, `questions=${JSON.stringify(questions)}&user=${JSON.stringify(dataUser)}` )
        console.log(payload.success)
        setShowLoadSpin(false)
        if (payload.success === true) {
            fetchLeads(true, backendURLBase, endpoints, clientId, dataUser, emailData,questions)
            setShowEmailForm(true)
            setShowThankYou(false)
        }
        if(payload.success !== true){
        fetchLeads(false, backendURLBase, endpoints, clientId, dataUser, emailData,questions)
